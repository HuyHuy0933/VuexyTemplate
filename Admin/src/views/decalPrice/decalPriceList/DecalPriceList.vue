<template>
  <b-card>
    <b-card-header>
      <h4 class="mb-0">Mẫu khuôn bế</h4>
      <b-card-text class="font-medium-5 mb-0">
        <decal-price-form-modal
          ref="decalPriceFormModal"
        ></decal-price-form-modal>
      </b-card-text>
    </b-card-header>
    <b-card-body>
      ...
      >
        <template slot="table-row" slot-scope="props">
          ...
          <span
            v-else-if="props.column.field === 'printPrice'"
            class="text-nowrap"
          >
            {{ formatCurrency(props.row.printPrice) }}
          </span>
          <span
            v-else-if="props.column.field === 'cutPrice'"
            class="text-nowrap"
          >
            {{ formatCurrency(props.row.cutPrice) }}
          </span>
          <span
            v-else-if="props.column.field === 'machiningPrice'"
            class="text-nowrap"
          >
            {{ formatCurrency(props.row.machiningPrice) }}
          </span>
...
<script src="./DecalPriceList.ts"></script>
